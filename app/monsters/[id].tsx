...
export default function MonsterDetailScreen() {
  ...
  useEffect(() => {
    const fetchMonsterDetails = async () => {
      try {
        const response = await axios.get<MonsterDetails>(`https://mhw-db.com/monsters/${id}`);
  ...
  return (
    <ScrollView style={styles.container}>
      {monsterDetails.icon && !imageError ? (
        <Image 
        ...
        <View style={[styles.image, styles.placeholderImage]}>
          <Text style={styles.placeholderText}>{monsterDetails.name[0]}</Text>
        </View>
      )}
